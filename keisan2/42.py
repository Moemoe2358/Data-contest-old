...
for i in range(len(test)):
	...
	if length == 2:
		submit["b"][i] = weight[natural][location][area] * temp["y"][1] + 1.4 * (1 - weight[natural][location][area]) * temp["y"][0]
	...
	# lel.append(le)
	# ale = (np.log(temp['y'][length - 1] + 1) - np.log(weight[natural][location][area] * temp['y'][length - 2] + (1 - weight[natural][location][area]) * temp['y'][length - 3] + 1)) ** 2
	# alel.append(ale)
