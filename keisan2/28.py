...
for i in range(len(test)):
	...
	if length >= 3:
		submit["b"][i] = 0.45 * temp["y"][length - 1] + 0.33 * temp["y"][length - 2] + 0.22 * temp["y"][length - 3]
	...
	lel.append(le)
	ale = (np.log(temp['y'][length - 1] + 1) - np.log(0.45 * temp['y'][length - 2] + 0.33 * temp['y'][length - 3] + 0.22 * temp['y'][length - 4] + 1)) ** 2
	alel.append(ale)
